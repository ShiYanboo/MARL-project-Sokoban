...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run9.3_bfs_nodl_nopush_rnn_$(date +%Y%m%d_%H%M%S).log"
...
CUDA_VISIBLE_DEVICES=4 PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --exp_name happo_turn_based_bfs_nodl_nopush_rnn \
  --run_name_prefix happo-rnn-bfs-nodl-nopush \
  --cuda True \
