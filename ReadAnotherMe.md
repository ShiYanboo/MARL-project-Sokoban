...
- 改环境封装：严格 turn-based、inactive agent 只能 noop、补充 action mask 和 Sokoban 指标。
- 改奖励：旧 BFS shaping、v2 reverse-push/useful-position shaping、deadlock/pushability/agent-box 等 ablation。
- 改网络：MLP、CNN、GRU、动作历史。
- 改训练形态：7x7 独训、历史伪混合、13x13 padding、true v0/v1/v2 mix、7x7 到 13x13 distillation。

目前最稳的经验结论是：

- `v1-nodl-nopush` 比原始 baseline 有一点优势。
- CNN 没有成为稳定提升点。
- GRU 在 7x7 上有希望，但 13x13/generalization 仍难。
- v2 奖励更符合 Sokoban 语义，但训练上不如旧 BFS 稳。
- distillation 到 13x13 没有成功保住 7x7 策略能力。

因此新加的 `run16/` 只做一个很小的算法层 credit-assignment 检查。如果 `run16` 也没有明显优势，就应该停止继续魔改，转入大作业报告整理。
...
- `run0/`：原始奖励 baseline，`reward_finished=10`，无 shaping。
- `run2/`：早期 HAPPO 超参数 sweep。
- `run7/`：reward shaping v2 与 cap ablation。
- `run8/`：朴素 BFS shaping 已筛选方案的继续训练与 v0/v1/v2 混合训练
- `run9/`：GRU/RNN + 动作历史 observation 的实验
- `run10/`：v2 cap12 的长程续训与历史伪混合。
- `run11/`：GRU no-history / one-action-history 的补齐实验。
- `run12/`：true-mix padded smoke tests。
- `run13/`：7x7 到 13x13 distillation 尝试，结果不理想。
- `run14/`：13x13 random padding 从头训练。
- `run15/`：13x13 top-left padding curriculum。
- `run16/`：actor-side credit assignment 小型算法实验。

每个 `run*/` 子目录都放了一个 `README.md`。如果忘了某个系列是干什么的，先看对应 README。
...
如果只想验证最新的 actor-side credit assignment 小改动：

```bash
CUDA_VISIBLE_DEVICES=5 bash run16/run_general16_credit_assignment.sh
```

它会顺序跑三条 5M 对照：

- `active_progress, coef=0.05`
- `active_progress, coef=0.10`
- `active_reward, coef=0.02`
...
### 2.3 Actor-side credit assignment

当前 `r1=r2=r` 默认分配不是一个明显 bug。原因是：

- 我们的环境是 turn-based。
- inactive agent 只有 noop 可选。
- inactive agent 的 active mask 为 0，不参与该步 policy loss。
- HAPPO 的 central critic 学的是 team return。

也就是说，默认共享团队奖励仍然是一个合理的协作目标。

问题在于训练信号方差很大：active agent 做了一个动作后，最终 success/box completion 可能很多步以后才体现出来。为了测试是否能缓解这个问题，新增了一个默认关闭的 actor-side credit：
...
默认 `none + 0.0`，所有旧脚本完全不变。

开启后，critic 仍然吃原始 team reward；只有 actor 的 advantage 会多一个很小的 active-agent credit：

```text
actor_advantage = team_advantage + actor_credit_coef * active_agent_credit
```

`active_progress` 会奖励 active agent 的即时正向进展，例如正 base reward、有用推箱距离下降、靠近有效推箱站位、移动箱子等。`active_reward` 则只是把 team reward 的一个小比例给 active agent 的 actor advantage。

这不是严格无偏的原始 PPO 目标，而是辅助 loss/advantage shaping。它的价值只在于做一个廉价 ablation：如果 run16 没有效，就别继续在算法层面深挖了。
