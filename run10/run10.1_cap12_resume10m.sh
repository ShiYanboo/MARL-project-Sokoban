#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/.." && pwd)
cd "$REPO_ROOT"

LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run10.1_cap12_resume10m_$(date +%Y%m%d_%H%M%S).log"
echo "Logging to $LOGFILE"

MODEL_DIR=$(find results/happo-shaped-v2 -type d -path '*/models' \
  | grep 'happo_turn_based_shaped_v2_conservative_cap12' \
  | grep -- '-lr1e-4-' \
  | xargs -r stat -c '%Y %n' \
  | sort -n \
  | tail -1 \
  | cut -d' ' -f2- || true)
if [[ -z "${MODEL_DIR:-}" || ! -d "$MODEL_DIR" ]]; then
  echo "Could not find source cap12 lr1e-4 model directory under results/happo-shaped-v2" >&2
  exit 1
fi
echo "Loading model from $MODEL_DIR"

CUDA_VISIBLE_DEVICES=4 PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  --algo happo \
  --env sokoban \
  --scenario TwoPlayer-Sokoban-v0 \
  --exp_name happo_turn_based_shaped_v2_conservative_cap12_resume10m \
  --run_name_prefix happo-shaped-v2-cap12-resume10m \
  --model_dir "$MODEL_DIR" \
  --cuda True \
  --control_mode turn_based \
  --observation_type vector \
  --n_rollout_threads 32 \
  --n_eval_rollout_threads 16 \
  --episode_length 150 \
  --num_env_steps 5000000 \
  --max_steps 150 \
  --dim_room 7 \
  --num_boxes 2 \
  --reward_finished 100 \
  --lr 1e-4 \
  --critic_lr 3e-4 \
  --entropy_coef 0.02 \
  --ppo_epoch 5 \
  --clip_param 0.1 \
  --hidden_sizes "[512, 512]" \
  --use_reward_shaping True \
  --box_target_distance_mode reverse_push \
  --agent_box_distance_mode useful \
  --distance_shaping_weight 0.08 \
  --pushability_shaping_weight 0.0 \
  --deadlock_penalty 0.0 \
  --deadlock_penalty_mode increase \
  --agent_box_distance_shaping_weight 0.02 \
  --useful_push_shaping_weight 0.0 \
  --shaping_unreachable_distance 12 \
  --log_interval 5 \
  --eval_interval 40 \
  --eval_episodes 20 \
  --use_linear_lr_decay True \
  --use_eval True 2>&1 | tee -a "$LOGFILE"
