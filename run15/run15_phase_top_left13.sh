...
if [[ $# -lt 3 ]]; then
  ...
  echo "Families: baseline-original, v1-noshape, v1-nodl-nopush, v1-deadlock005, v1-deadlock020, gru-noaction, gru-8action, gru-1action, cnn-nodl-nopush" >&2
  exit 2
fi
...
case "$FAMILY" in
  baseline-original)
    EXP_NAME="happo_top_left13_baseline_original_${PHASE}"
    REWARD_FINISHED=10
    USE_REWARD_SHAPING=False
    ;;
  v1-noshape)
    EXP_NAME="happo_top_left13_v1_noshape_${PHASE}"
    USE_REWARD_SHAPING=False
    ;;
  v1-nodl-nopush)
    EXP_NAME="happo_top_left13_v1_nodl_nopush_${PHASE}"
    ;;
  v1-deadlock005)
    EXP_NAME="happo_top_left13_v1_deadlock005_${PHASE}"
    PUSHABILITY_WEIGHT=0.05
    DEADLOCK_PENALTY=0.05
    ;;
  v1-deadlock020)
    EXP_NAME="happo_top_left13_v1_deadlock020_${PHASE}"
    PUSHABILITY_WEIGHT=0.05
    DEADLOCK_PENALTY=0.20
    ;;
  gru-noaction)
    EXP_NAME="happo_top_left13_gru_noaction_${PHASE}"
    USE_RECURRENT=True
    ROLLOUT_THREADS_DEFAULT=48
    ;;
  gru-8action)
    EXP_NAME="happo_top_left13_gru_8action_${PHASE}"
    USE_RECURRENT=True
    ROLLOUT_THREADS_DEFAULT=48
    ACTION_HISTORY_LEN=8
    ;;
  gru-1action)
    EXP_NAME="happo_top_left13_gru_1action_${PHASE}"
    USE_RECURRENT=True
    ROLLOUT_THREADS_DEFAULT=48
    ACTION_HISTORY_LEN=1
    ;;
  cnn-nodl-nopush)
    EXP_NAME="happo_top_left13_cnn_nodl_nopush_${PHASE}"
...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run15_${FAMILY}_${PHASE}_$(date +%Y%m%d_%H%M%S).log"
