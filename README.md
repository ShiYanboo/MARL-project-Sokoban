...
- `HARL/harl/envs/sokoban/sokoban_logger.py`
  Sokoban 的 on-policy 日志器，负责额外记录成功率、冲突率、推箱次数等统计
...
- 环境先接收两个 agent 的动作，再根据冲突规则挑一个实际执行
- 当前默认冲突规则是 `round_robin`
...
- `HARL/results/sokoban/TwoPlayer-Sokoban-v0/happo/trial_happo/seed-00001-2026-06-20-17-35-26/`
...
```powershell
python HARL\scripts\plot_sokoban_metrics.py --run-dir HARL\results\sokoban\TwoPlayer-Sokoban-v0\happo\trial_happo\seed-00001-2026-06-20-17-35-26
```
...
- `HARL/results/sokoban/TwoPlayer-Sokoban-v0/happo/trial_happo/seed-00001-2026-06-20-17-35-26/plots/reward_curves.png`
- `HARL/results/sokoban/TwoPlayer-Sokoban-v0/happo/trial_happo/seed-00001-2026-06-20-17-35-26/plots/optimization_curves.png`
...
```powershell
tensorboard --logdir HARL\results
```

Linux setup 脚本里已经补上了 `tensorboard` 和 `matplotlib`。
...
4. 根据 `round_robin` 或 `random` 冲突规则选一个真正执行
5. 若选中 agent1，则动作会映射到底层双人 Sokoban 的 `9~16`
6. 底层环境真正 step 一次
