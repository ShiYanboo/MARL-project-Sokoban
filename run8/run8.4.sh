...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run8.4_$(date +%Y%m%d_%H%M%S).log"
echo "Logging to $LOGFILE"

MODEL_DIR=$(find results/happo-shaped-nodeadlock-resume10m -path '*/models' -type d -printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2- || true)
if [[ -z "${MODEL_DIR:-}" || ! -d "$MODEL_DIR" ]]; then
  echo "Could not find source model directory under results/happo-shaped-nodeadlock-resume10m" >&2
  exit 1
fi
...
CUDA_VISIBLE_DEVICES=5 PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --exp_name happo_turn_based_shaped_nodeadlock_resume15m \
  --run_name_prefix happo-shaped-nodeadlock-resume15m \
  --model_dir "$MODEL_DIR" \
  --cuda True \
