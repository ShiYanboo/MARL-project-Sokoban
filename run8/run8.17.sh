...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run8.17_$(date +%Y%m%d_%H%M%S).log"
echo "Logging to $LOGFILE"

MODEL_DIR=""
for PREFIX in happo-shaped-nodl-nopush-resume15m happo-shaped-nodl-nopush-resume10m happo-shaped-nodl-nopush-resume5m; do
...
if [[ -z "${MODEL_DIR:-}" || ! -d "$MODEL_DIR" ]]; then
  echo "Could not find source model directory under nodl-nopush resume15m/resume10m/resume5m" >&2
  exit 1
fi
...
CUDA_VISIBLE_DEVICES=5 PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --exp_name happo_turn_based_shaped_nodl_nopush_mixv012_r1 \
  --run_name_prefix happo-shaped-nodl-nopush-mixv012-r1 \
  --model_dir "$MODEL_DIR" \
  --cuda True \
