...
\caption{7x7、2 boxes 固定环境下的主线长跑结果，比较 HAPPO baseline 与若干轻量塑形/架构变体。曲线显示各方案均能脱离随机策略并持续上升；其中去掉 deadlock 与 pushability 负项的 \code{v1-nodl-nopush} 与 \code{gru0} 收敛最稳、波动最小，我们将后者作为提交的最终模型，而将前者置为大量消融实验中对比的基线。}
\label{fig:old7x7}
\end{figure*}
...
\subsection{Credit Assignment and Actor-Side Credit}
...
\subsection{HAHyPO: GRPO-Style Advantage}
...
\caption{v2 reverse-push distance 的 unreachable-distance cap 消融。\code{v2-clip12} 在短程内表现最好，说明对不可达距离做裁剪、控制塑形信号尺度十分重要。}
\label{fig:v2clip}
\end{figure}
...
\section{结果与分析}
\subsection{主结果}
图 \ref{fig:old7x7} 给出 7x7、2 boxes 固定环境下的主线长跑结果，比较对象集中在 HAPPO baseline 与 \code{v1-nodl-nopush}。结果表明，HAPPO 在该难度下可以明显脱离随机策略，训练成功率与 box completion ratio 在长程内持续上升。在所有轻量方案中，保留正向势函数、去掉 deadlock 与 pushability 负项的 \code{v1-nodl-nopush} 收敛最稳、波动最小。因此我们把 \code{v1-nodl-nopush} 作为后续各项消融的统一参照。
...
\caption{动作历史长度消融。显式输入动作历史并不稳定优于无历史，\code{gru-8} 不及 \code{gru-0}，\code{mlp-8} 也劣于 \code{gru-8}。这说明 GRU 架构的时序建模能力能够使得效果提升，但显式增加动作输入并无益处。}
\label{fig:history}
\end{figure}
...
对这一现象的解释如下：带有动作历史的非 Markov 环境可以被 Markov 化，因此从表示能力上讲并无区别；引入动作序列有可能使得“理解另一智能体的行动逻辑”更为容易，但额外的历史输入会增大学习难度。在两者的trade-off中，历史动作序列的输入几乎没有影响或有负作用。通过图 \ref{fig:history}也可以看出， GRU 带来的提升更可能来自其时序建模能力本身，而不是来自动作历史这一额外输入。
...
\subsection{HAHyPO}
\label{sec:hahypo}
图 \ref{fig:hahypo} 给出 HAHyPO 在 \code{v1-nodl-nopush} 上的 20M 对照。结果表明，引入 GRPO-style 优势并没有提升性能：小权重 $\alpha=0.2$ 基本跟随 HAPPO，$\alpha=0.5$ 明显偏弱，$\alpha=1.0$ 接近失败。其弱点在于轨迹级相对排序信号过粗，难以替代逐步 advantage，这与近期LLM RL回归PPO的观察相一致：PPO系列比GRPO系列优化更稳健，在智能体任务中这一特性更加突出。
...
\textbf{\code{top-left padding}} 仍然困难，但 CNN 最优，其 train/eval success 与 eval box completion 都显著高于 MLP/GRU，这一结果显著体现了 CNN 的空间归纳偏置与平移不变性优势。一个值得强调的观察是：在原始等大输入（7*7）实验中，曲线通常很快离开起始阶段，因此早期趋势对最终表现有较强指示；但在 13x13 top-left padding 中，训练会出现明显更长的平台期（大约$5e6$步），random padding 难以训练可能由于其 warmup 平台更长、甚至超过了我们设置的训练总步数。显然 top-left padding 并没有增加有效信息，只是扩大了网络输入容量，但模型仍需通过训练学会哪些位置是无信息的——对观察者而言，这相当于只能通过贝叶斯式的经验推断判断``某些输入区域不会带来信息''。因此，仅扩充输入规模即使不增加有效信息也可能显著提高优化难度；容量提升通常需要与更丰富、更优质的训练信号匹配。
...
\subsection{动作不合法惩罚}
作为行为先验的辅助分析，我们考察了无用动作惩罚项（3.2节）。图 \ref{fig:useless}结果显示，该惩罚确实降低了 useless-action rate，并提高了 agent 的移动次数，说明奖励项改变了底层行为分布，使 agent 从``不动/浪费动作''转向更多行动。但其成功率与箱子到位率没有超过 \code{v1-nodl-nopush}，长程潜力反而较弱。这说明限制性能的主要因素并不是``没有合理动作''或``动作不够活跃''，而是推箱方向选择、长期规划与避免不可逆的错误 push。因此，无用动作惩罚验证了低层行为先验可以减少无效动作，但``多动''不等于会解 Sokoban，它应被理解为加速早期探索的行为先验，而非与任务完成直接相关的正向信号。

\section{结论与启发}
综合现有材料，我们得到以下结论：
\begin{enumerate}[leftmargin=1.4em,itemsep=0.25em]
    \item 主结果：HAPPO 在 7x7、2 boxes 下可以稳定学到协作策略；保留正向势函数、去掉 deadlock 与 pushability 负项的 \code{v1-nodl-nopush} 是当前实现中较稳的主线。
    \item 距离语义：更符合推箱语义的 reverse-push v2 并未带来更高成功率，更精确的 reward 反而使优化更崎岖；不可达距离的 cap 对控制信号尺度十分关键。
    \item 模型架构：GRU 表现最好，显式动作历史会降低 GRU 性能，\code{mlp-8} 也劣于 \code{gru-8}；显式动作输入不会使得模型能力上升；CNN 在 7x7 不占优。
    ...
    \item Mix：13x13 random padding 几乎无法训练，top-left padding 虽仍困难但 CNN 明显最优，体现了空间归纳偏置与平移不变性的价值；distillation 受限于模型容量而未见效果。若追求泛化，应优先使用CNN与课程学习，而不是单点调 reward。
    \item 动作不合法惩罚：施加惩罚后没有明显优势，表明限制模型能力的是问题的复杂性而非动作的多样性，该惩罚项只能加速初始探索有效性但无法提供导向结果的正向信号。
\end{enumerate}

\begin{figure}[!tbp]
\centering
\includegraphics[width=\columnwidth]{figures/18_v1_useless_action_25m.png}
\caption{illegal action punishment的消融实验。当惩罚权重非零时，智能体的行动率更高，与惩罚强度正相关；但施加惩罚后成功率没有明显超过 \code{v1-nodl-nopush}，后期朴素\code{v1-nodl-nopush}反倒更具潜力。}
\label{fig:useless}
\end{figure}
