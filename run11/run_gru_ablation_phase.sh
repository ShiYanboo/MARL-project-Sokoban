...
if [[ $# -lt 5 ]]; then
  echo "Usage: $0 <variant_label> <action_history_len> <phase_label> <run_name_prefix> <exp_name> [source_prefix] [scenario_pool]" >&2
  exit 2
fi
...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run11_${VARIANT_LABEL}_${PHASE_LABEL}_$(date +%Y%m%d_%H%M%S).log"
