...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run16_1_credit_progress005_$(date +%Y%m%d_%H%M%S).log"
...
PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --exp_name happo_credit_assignment \
  --run_name_prefix happo-credit-progress005 \
  --cuda True \
  ...
  --actor_credit_mode active_progress \
  --actor_credit_coef 0.05 \
  --log_interval 5 \
  --eval_interval 50 \
  --eval_episodes 20 \
