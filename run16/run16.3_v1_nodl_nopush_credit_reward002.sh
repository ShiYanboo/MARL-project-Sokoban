...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run16_3_credit_reward002_$(date +%Y%m%d_%H%M%S).log"
...
PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --run_name_prefix happo-credit-reward002 \
  --cuda True \
  ...
  --actor_credit_mode active_reward \
  --actor_credit_coef 0.02 \
  --log_interval 5 \
  --eval_interval 50 \
  --eval_episodes 20 \
