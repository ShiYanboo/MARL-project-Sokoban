...
LOG_DIR=log
mkdir -p "$LOG_DIR"
LOGFILE="$LOG_DIR/run6.3_$(date +%Y%m%d_%H%M%S).log"
...
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python -u HARL/examples/train.py \
  ...
  --exp_name happo_turn_based_shaped_nodl_nopush_nodist \
  --run_name_prefix happo-shaped-nodl-nopush-nodist \
  --cuda True \
